...
Download `Scanio-0.5.0-beta.2-win-x64-setup.exe` and `SHA256SUMS.txt` from [GitHub Releases](https://github.com/thevladbog/scanio/releases/tag/v0.5.0-beta.2).
...
`Scanio-0.5.0-beta.2-win-x64-portable.zip` is the no-install alternative. Verify its SHA-256 checksum, extract the complete ZIP to a writable directory, and run `Scanio.exe`. Keep `portable.flag` and the adjacent `Data` directory with the application; portable Notebook sessions are stored in `Data/scanio.db`.
