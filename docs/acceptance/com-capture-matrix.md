...
6. Hold the port open in another application, manually connect in Scanio, and confirm **Port busy** without automatic retry.
7. Disconnect the cable while connected. Confirm **Device removed**, responsive UI, and no automatic reconnect.
8. Press **Disconnect**, then immediately open the port in another application. Confirm success.
9. Connect again, exit Scanio, then immediately open the port elsewhere. Confirm success.
10. Repeat at least once after Windows assigns a different COM number. Stable identity may persist only when the device exposes a real serial number.
